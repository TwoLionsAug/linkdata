...
//        response.setContentType("application/vnd.ms-excel"); // 改成输出excel文件
     ...
     * @return 字段
     */
    public static String getFieldByName(String fieldName, Object o) {
        // 拿到本类的所有字段
//        JSONObject jsonObject = JSONObject.parseObject(fieldName);
//
...
//        Field field = getFieldByName(fieldName, o.getClass());
        String string = getFieldByName(fieldName, o);
        //如果该字段存在，则取出该字段的值
        if (string != null) {
            String string2 = JSON.toJSONString(o, SerializerFeature.WriteMapNullValue);
            String string3 = string2.replaceAll("null","''");
            JSONObject parse = JSON.parseObject(string3);
            if(fieldName.indexOf("time") != -1){
                if(!StringUtils.isEmpty(parse.get(fieldName).toString())){
                    SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    long lt = Long.parseLong(parse.get(fieldName).toString());
                    value = formatter.format(new Date(lt));
                }else{
                    value = parse.get(fieldName);//获取当前对象中当前Field的value
                }
            }else{
                value = parse.get(fieldName);//获取当前对象中当前Field的value
            }
